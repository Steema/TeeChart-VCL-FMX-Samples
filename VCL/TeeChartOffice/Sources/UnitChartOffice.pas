...
uses
  ...
  TeeExpression,
  TeeAxisDivider, TeeExtraLegendTool, TeeSurfaceTool, TeeDataTableEditor,
...
type
  ...
  TMainForm = class(TForm)
    ...
    Import1: TMenuItem;
    GridMode1: TMenuItem;
    SeriesTitle1: TMenuItem;
    PointIndex1: TMenuItem;
    PercentRelative1: TMenuItem;
    LabelPercentValue1: TMenuItem;
    LabelorValue1: TMenuItem;
  ...
  private
    ...
    Function SelectedTitleName(const APart:TChartClickedPart):String;
...
begin { set-up the Gallery with a list of Series styles }
  ...
  begin
    ...
    begin { create sub-gallery }
      with TChartGalleryAccess(ChartGalleryPanel1) do
      begin
...
begin { check what Chart part is under the mouse, and show it at the status bar }
  ...
  DBChart1.CalcClickedPart(TeePoint(X,Y),tmpPart);

  Case tmpPart.Part of
        cpLegend:  S:=TeeMsg_Legend;
         cpAxis:   S:=DBChart1.AxisTitleOrName(tmpPart.AAxis);
         cpSeries: begin
                     S:=SeriesTitleOrName(tmpPart.ASeries)+' '+IntToStr(tmpPart.PointIndex);
                     StatusBar1.Invalidate;
                   end;
    cpSeriesMarks: S:=SeriesTitleOrName(tmpPart.ASeries)+' '+TeeMsg_Marks;
      cpSubTitle,
        cpTitle,
      cpSubFoot,
        cpFoot:   S:=SelectedTitleName(tmpPart);
  else
  begin
    S:='';
...
Function TMainForm.SelectedTitleName(const APart:TChartClickedPart):String;
...
var tmp:        TTeeFont;
   ...
   begin
     ...
     { select at Inspector }
     ComboBoxParts.Items[ComboPart_Title]:=SelectedTitleName(Selector.Part);
...
begin { draw selected point color at status bar }
  if (Panel=StatusBar1.Panels[1]) and
     Assigned(Selector) and
